...
`https://<raspiのマシン名>.<tailnet名>.ts.net/`のような形式とプレースホルダーは説明用の公開情報として記載できます。利用中の実際のURL、マシン名、tailnet名はGitHub Pagesへ載せません。実URLは許可された端末を持つ利用者へ、tailnet内のブックマークなどで個別に共有します。公開ページから実URLへ通常リンクを作ると、URL自体が公開情報になるため行いません。
