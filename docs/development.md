...
ChromeまたはChromiumがインストールされた開発環境では、`pytest`が実ブラウザーで動体検知枠の表示・色・映像との位置合わせ・パネル開閉も確認します。ブラウザーがない環境ではこのテストだけスキップします。テストは一時ファイルと模擬API応答を使い、カメラには接続しません。Pi上での実機確認とは区別してください。
...
| Picamera2の90度単位の映像回転、写真・MP4への反映、角度の永続化 | 実装済み（実機での表示・負荷は未確認） |
| 写真・動画履歴のページング、ダウンロード、削除 | 実装済み |
| 動体検知ON/OFFの永続化と状態表示 | 実装済み |
| 明るさ揺れの補正・同じ領域の連続判定・検知位置と録画開始理由の表示 | 実装済み（設置環境での精度・負荷は未確認） |
| 微小なカメラ揺れの位置補正・画素平均によるノイズ低減 | 実装済み（設置環境での精度・負荷は未確認） |
...
- 90・180・270度のライブ映像、JPEG写真、MP4再生と再起動後の角度維持
- LED照明・暗部での誤検知抑制と、小さな被写体の検知維持、診断枠の表示
- 棚などから伝わる振動の補正と、振動中の被写体の動きの検知
...
3. Picamera2利用時はライブ映像、回転、写真、録画、録画停止、保存制限、状態表示を確認する。
