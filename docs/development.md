...
- ライブ映像はカラーの低解像度MJPEGストリームとし、Pi 3Bでは既定で640x360・5fpsを上限の目安とする。
- 動体検知はライブ用低解像度RGBフレームから得た輝度を間引いた画素差分で行う。初期実装でOpenCVは導入しない。
- 全体の大半が同方向へ明暗変化した場合は照明変化として除外し、基準画像を更新して数秒間の待機後に検知を再開する。
- 通常の動きは、差分がしきい値を超えた画素の割合で判定する。これにより画面全体の平均差分より局所的な小動物の動きを調整しやすくする。
...
- 録画済み動画は`GET /api/videos`で一覧し、`GET /videos/{video_id}`を添付ファイルとしてダウンロードする。Picamera2モードの画面には動画一覧とダウンロード操作を表示する。
